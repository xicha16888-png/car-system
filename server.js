...
// ══ 实际月利息率 = 综合利息占实际放款的比例，平摊到每个月 ══
// 砍头息是放款那一刻一次性收的，但它本质上也是这笔钱的利息成本，只是提前收了；
// 把"砍头息+整期利息"这笔综合利息，平摊到合同的每一期（每个月），才是这笔单子
// 真正的月利息率——比单看"月利率%"这个字段（只算了分期利息，没算砍头）更准。
function loanEffectiveMonthlyRate(l) {
  var term = l.term || 1;
  return Math.round(loanComprehensiveInterestPct(l)/term*100)/100;
}
...
function calcPreview() {
  ...
  var effMonthlyRate = term ? Math.round(comprehensivePct/term*100)/100 : 0;
  ...
  box.innerHTML =
    ...
    "<div style=\"background:#ffebee;border-radius:8px;padding:10px;text-align:center;border:1.5px solid #ef9a9a\"><div style=\"font-size:11px;color:#546e7a\">🧮 实际月利率（砍头+总利息平摊到每月）</div><div style=\"font-size:18px;font-weight:700;color:#c62828\">"+effMonthlyRate+"%</div><div style=\"font-size:10px;color:#546e7a\">综合利息 $"+comprehensive.toFixed(2)+"（占实放"+comprehensivePct+"%）</div></div>" +
    "</div>" +
...
function renderActualSchedule(d) {
  ...
  var effMonthlyRate = loanEffectiveMonthlyRate(d);
  box.style.display = "block";
  box.innerHTML =
  ...
  if(mobile) {
    ...
    mkeys.forEach(function(mk){
      ...
      gloans.forEach(function(l){
        ...
        h+='<div class="loan-card-row"><span class="loan-card-lbl">🧮 实际月利率</span><span class="loan-card-val" style="color:#c62828;font-weight:700">'+loanEffectiveMonthlyRate(l)+'%/月 · 综合$'+loanComprehensiveInterest(l).toFixed(2)+'</span></div>';
    ...
    mkeys.forEach(function(mk){
      ...
      h+='<th>#</th><th>客户姓名</th><th>手机号</th><th>车牌 / 品牌</th><th>贷款金额</th><th>实际放款</th><th>砍头%</th><th>月利率%</th><th>月利息</th><th>实际月利率</th><th>还款方式</th><th>状态</th><th>放款日</th><th>操作</th>';
      h+='</tr></thead><tbody>';
      gloans.forEach(function(l){
        ...
        h+='<td style="font-weight:700;color:#c62828;white-space:nowrap">'+loanEffectiveMonthlyRate(l)+'%/月<div style="font-size:10px;color:var(--muted);font-weight:400">综合$'+loanComprehensiveInterest(l).toFixed(2)+'</div></td>';
